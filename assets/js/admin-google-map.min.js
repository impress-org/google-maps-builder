...
$("#gmb_markers_group_"+n+"_title").val("Point "+n),$("#gmb_markers_group_"+n+"_lat").val(t),$("#gmb_markers_group_"+n+"_lng").val(o),get_editable_info_window(n,i),google.maps.event.addListener(i,"click",function(){get_info_window_content(n,i)})}/**
	 ...
	 * @param map_canvas
	 */
function initialize_map(e){lat_field=$("#gmb_lat_lng-latitude"),lng_field=$("#gmb_lat_lng-longitude");var a=lat_field.val()?lat_field.val():"",t=lng_field.val()?lng_field.val():"";zoom=parseInt($("#gmb_zoom").val()),lat_lng=new google.maps.LatLng(a,t);var o={zoom:zoom,streetViewControl:!1,styles:[{stylers:[{visibility:"simplified"}]},{elementType:"labels",stylers:[{visibility:"off"}]}]};map=new google.maps.Map(e[0],o),places_service=new google.maps.places.PlacesService(map),
...
""==marker_icon?(
//default icon
marker_icon_data=gmb_data.plugin_url+"assets/img/default-marker.png",$("#gmb_markers_group_"+index+"_marker").val(""),marker_label_data=""):(
//maps-icon
...
if(info_bubble.close(),e.length>1){t='<div id="infobubble-content"><p>Hmm, it looks like there are multiple places in this area. Please confirm which place you would like this marker to display:</p>';for(var o=0;o<e.length;o++)t+='<a class="marker-confirm-place"  data-place_id="'+e[o].place_id+'" data-name-address="'+e[o].name+", "+e[o].vicinity+'">'+e[o].name+"</a>";t+="</div>",
...
e.prop("checked")||$("#gmb_show_places2").prop("checked",!0),e.on("change",function(){$(".cmb2-id-gmb-search-radius").toggle(),$(".cmb2-id-gmb-places-search-multicheckbox").toggle(),"no"===$(this).val()?clear_search_markers():perform_places_search()})}/**
	 * Set Map Size
	 */
function set_map_size(){var e=$("#gmb_width_height-width").val(),a=$('input[name="gmb_width_height[map_width_unit]"]:checked').val(),t=$("#gmb_width_height-height").val();$("#map").css({width:e+a,height:t})}/**
	 * Set Zoom Control
	 */
...
$(".gmb-magnific-inline").magnificPopup({type:"inline",midClick:!0})}/**
...
//Width/Height
$("#gmb_width_height-width, #gmb_width_height-height").keyup(function(){delay(function(){set_map_size()},500)}),$('input[name="gmb_width_height[map_width_unit]"]').change(function(){set_map_size()})});//End Window Load
