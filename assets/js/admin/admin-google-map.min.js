...
//add data to fields
$("#gmb_markers_group_"+n+"_title").val("Point "+parseInt(n+1)),$("#gmb_markers_group_"+n+"_lat").val(t),$("#gmb_markers_group_"+n+"_lng").val(o),get_editable_info_window(n,i),google.maps.event.addListener(i,"click",function(){get_info_window_content(n,i)})}/**
	 * Map Intialize
	 *
	 ...
	 * @param map_canvas
	 */
function initialize_map(e){lat_field=$("#gmb_lat_lng-latitude"),lng_field=$("#gmb_lat_lng-longitude");var a=$(".live-latitude"),t=$(".live-longitude"),o=lat_field.val()?lat_field.val():"",i=lng_field.val()?lng_field.val():"";zoom=parseInt($("#gmb_zoom").val()),lat_lng=new google.maps.LatLng(o,i);var n={zoom:zoom,streetViewControl:!1,styles:[{stylers:[{visibility:"simplified"}]},{elementType:"labels",stylers:[{visibility:"off"}]}]};
//Setup Autocomplete field if undefined
if(map=new google.maps.Map(e[0],n),window.map=map,places_service=new google.maps.places.PlacesService(map),
//Handle Map Geolocation
navigator.geolocation&&"yes"===gmb_data.geolocate_setting&&""==i&&""==o?navigator.geolocation.getCurrentPosition(function(e){initial_location=new google.maps.LatLng(e.coords.latitude,e.coords.longitude),map.setCenter(initial_location),//set map with location
lat_field.val(e.coords.latitude),//set lat field
lng_field.val(e.coords.longitude),//set lng field
a.text(e.coords.latitude),//update toolbar
t.text(e.coords.longitude)}):""!==o&&""!==i?
//set map with saved lat/lng
map.setCenter(new google.maps.LatLng(o,i)):(initial_location=new google.maps.LatLng(gmb_data.default_lat,gmb_data.default_lng),lat_field.val(gmb_data.default_lat),//set lat field
lng_field.val(gmb_data.default_lng),//set lng field
a.text(gmb_data.default_lat),//update toolbar
t.text(gmb_data.default_lng),//update toolbar
map.setCenter(initial_location)),
//Set various map view options
set_map_type(!1),"none"!==$("#gmb_theme").val()&&set_map_theme(!1),set_street_view(),set_pan_control(),set_draggable(),set_double_click_zoom(),set_mouse_wheel_scroll(),set_map_type_control(),set_map_zoom_control(),"undefined"==typeof autocomplete){var r=$("#gmb_geocoder");autocomplete=new google.maps.places.Autocomplete(r[0]),autocomplete.bindTo("bounds",map),
//Tame the enter key to not save the widget while using the autocomplete input
google.maps.event.addDomListener(r[0],"keydown",function(e){13==e.keyCode&&e.preventDefault()}),
...
//map Zoom Changed
google.maps.event.addListener(map,"zoom_changed",function(){handle_map_zoom(map)})}//end initialize_map
/**
...
//place name
info_window_data.title&&(info_window_content='<input class="edit-place-title" data-field="#gmb_markers_group_'+index+'_title" type="text" value="'+info_window_data.title+'">'),info_window_content+=info_window_data.desc?'<textarea class="edit-place-description" data-field="#gmb_markers_group_'+index+'_description">'+info_window_data.desc+"</textarea>":'<textarea class="edit-place-description" data-field="#gmb_markers_group_'+index+'_description"></textarea>',
//toolbar
info_window_content+='<div class="infowindow-toolbar clear"><ul id="save-toolbar"><li class="info-window-save"><div class="google-btn-blue google-btn google-save-btn" data-tooltip="Save changes" data-index="'+index+'">Save</div></li><li class="info-window-cancel"><div class="google-btn-default google-btn google-cancel-btn" data-tooltip="Cancel edit" data-index="'+index+'">Cancel</div></li></ul><span class="marker-edit-link-wrap" data-index="'+index+'"><a href="#" data-target="marker-icon-modal"  data-tooltip="Change icon"  data-mfp-src="#marker-icon-modal" class="marker-edit-link gmb-magnific-marker gmb-magnific-inline"></a></span></div>',info_window_content=set_info_window_wrapper(info_window_content),info_bubble.setContent(info_window_content),initialize_tooltips(),//refresh tooltips
...
"mapicons"==marker_icon||"upload"==marker_icon||"default"==marker_icon?(marker_icon_data=$(this).data("marker-image"),marker_label_data="",//no label here (img marker)
$("#gmb_markers_group_"+index+"_marker_img").val(marker_icon_data),//set marker image field
$("#gmb_markers_group_"+index+"_marker","#gmb_markers_group_"+index+"_label").val("")):("MAP_PIN"==marker_icon||"SQUARE_PIN"==marker_icon)&&(
//maps-icon
...
$("#gmb_markers_group_"+index+"_marker").val(marker_icon_data),$("#gmb_markers_group_"+index+"_label").val(marker_label_data),marker_icon_data=eval("("+marker_icon_data+")"),$("#gmb_markers_group_"+index+"_marker_img").val("")),
...
console.log("here"),console.log($(".magnific-builder").length),0===$(".magnific-builder").length?$.magnificPopup.close():$(".gmb-modal-close").trigger("click"),google.maps.event.removeListener(save_icon_listener),//remove this event listener
google.maps.event.removeListener(edit_marker_icon_button_click)})}/**
...
var location_marker=new Marker({map:map,zIndex:9,icon:marker_icon,custom_label:marker_label}),marker_lat=$("#gmb_markers_group_"+index+"_lat").val(),marker_lng=$("#gmb_markers_group_"+index+"_lng").val();location_marker.setPosition(new google.maps.LatLng(marker_lat,marker_lng)),location_marker.setVisible(!0),google.maps.event.addListener(location_marker,"click",function(){get_info_window_content(index,location_marker)})})}function get_info_window_saved_data(e){var a={};return a.title=$("#gmb_markers_group_"+e+"_title").val(),a.desc=$("#gmb_markers_group_"+e+"_description").val(),a.reference=$("#gmb_markers_group_"+e+"_reference").val(),a.place_id=$("#gmb_markers_group_"+e+"_place_id").val(),a.lat=$("#gmb_markers_group_"+e+"_lat").val(),a.lng=$("#gmb_markers_group_"+e+"_lng").val(),a.hide_place_info=$("#gmb_markers_group_"+e+"_hide_details").prop("checked"),a}/**
	 ...
	 * @param marker
	 */
function get_info_window_content(e,a){info_bubble.close(),info_bubble.setContent('<div id="infobubble-content" class="loading"></div>'),info_bubble.open(map,a);var t=get_info_window_saved_data(e),o='<p class="place-title">'+t.title+"</p>";
//Show place information within this infowindow?
if(o+='<div class="place-description">'+t.desc+"</div>",t.place_id&&t.hide_place_info===!1){var i={key:gmb_data.api_key,placeId:t.place_id};places_service.getDetails(i,function(t,i){i==google.maps.places.PlacesServiceStatus.OK&&(o+=add_place_content_to_info_window(t),o+=set_marker_edit_icons(e),add_edit_events(o,a))})}else o+=set_marker_edit_icons(e),add_edit_events(o,a)}/**
	 * Add Edit Events
	 *
...
//place name
t='<p class="place-title">'+e.name+"</p>",t+=add_place_content_to_info_window(e),t+=add_mar(e),t=set_info_window_wrapper(t),//wraps the content in div and returns
...
e.prop("checked")||$("#gmb_show_places2").prop("checked",!0),
//Places
$(".cmb2-id-gmb-show-places li input:radio").on("click",function(){$(this).find("input:radio").prop("checked",!0),"no"===$(this).val()?(clear_search_markers(),$(".cmb2-id-gmb-search-radius, .cmb2-id-gmb-places-search-multicheckbox").hide()):(perform_places_search(),$(".cmb2-id-gmb-search-radius, .cmb2-id-gmb-places-search-multicheckbox").show())})}/**
	 * Set Zoom Control
	 */
...
$.getJSON(gmb_data.snazzy,function(data){var map_theme_input_val=parseInt($("#gmb_theme").val());"none"===$("#gmb_theme").val()?(set_map_type(),$(".cmb2-id-gmb-theme-json").hide()):$(".cmb2-id-gmb-theme-json").show(),$.each(data,function(index){data[index].id===map_theme_input_val&&(map_theme_input_val=eval(data[index].json),$("#gmb_theme_json").val(data[index].json))}),map.setOptions({mapTypeId:google.maps.MapTypeId.ROADMAP,styles:map_theme_input_val})})}/**
	 ...
	 */
function set_map_marker_icon(){var e=$(".marker-icon-row"),a=$(".marker-icon-modal"),t=a.find(".save-marker-icon"),o=a.find(".save-marker-button");
//Marker Item Click
$(".marker-item").on("click",function(){var a=$(this).data("marker"),t=$(this).data("toggle");$(".marker-item").removeClass("marker-item-selected"),$(this).addClass("marker-item-selected"),o.attr("data-marker",a),//Set marker data attribute on save bt
//Slide up all panels
e.hide(),
//Slide down specific div
$("."+t).slideDown()}),
//Old school icon click action
$(".maps-icon").on("click",function(){$(".maps-icon").removeClass("marker-item-selected"),t.slideDown(),$(this).addClass("marker-item-selected"),o.data("marker-image",$(this).find("img").attr("src"))}),
//SVG/Font icon Click
...
o.attr("data-label",$(this).find("span").attr("class"))});
//Setup colorpickers
var i={
...
palettes:!0};$(".color-picker").wpColorPicker(i)}/**
	 ...
	 * @returns {string}
	 */
function set_marker_edit_icons(e){return'<div class="infowindow-toolbar"><ul id="edit-toolbar"><li class="edit-info" data-index="'+e+'" data-tooltip="'+gmb_data.i18n.btn_edit_marker+'"></li><li class="trash-marker" data-index="'+e+'" data-tooltip="'+gmb_data.i18n.btn_delete_marker+'"></li></ul></div>'}/**
	 * Refresh Tooltips
	 *
...
//click to add marker
$(".drop-marker").on("click",function(e){if(e.preventDefault(),$(this).hasClass("active"))$(this).html(gmb_data.i18n.btn_drop_marker).removeClass("active"),map.setOptions({draggableCursor:null});else{$(this).html(gmb_data.i18n.btn_drop_marker_click).addClass("active"),map.setOptions({draggableCursor:"crosshair"});var a=google.maps.event.addListener(map,"click",function(e){drop_marker(e.latLng,a)})}});
//Radius Fields
var e;
//Search Radius Circle
$("#gmb_search_radius").on("focus",function(){google.maps.event.trigger(map,"resize"),//refresh map to get exact center
e=$(this).val(),calc_radius(map,parseInt($(this).val()))}).focusout(function(){e!==$(this).val()&&perform_places_search(),radius_circle.setMap(null),//removes circle on focus out
radius_marker.setMap(null)}),
//Places Type Field
$('[name^="gmb_places_search_multicheckbox"]').on("change",function(){
//Show message if not already displayed
0===$(".places-change-message").length&&($(".cmb2-id-gmb-places-search-multicheckbox ul").prepend('<div class="wpgp-message places-change-message clear"><p>'+gmb_data.i18n.places_selection_changed+'</p><a href="#" class="button update-places-map">'+gmb_data.i18n.set_place_types+"</a></div>"),$(".places-change-message").slideDown())}),$(".cmb-multicheck-toggle").on("click",function(){0===$(".places-change-message").length&&($(".cmb2-id-gmb-places-search-multicheckbox ul").prepend('<div class="wpgp-message places-change-message clear"><p>'+gmb_data.i18n.places_selection_changed+'</p><a href="#" class="button update-places-map">'+gmb_data.i18n.set_place_types+"</a></div>"),$(".places-change-message").slideDown())}),
...
$(".lat-lng-update-btn, .update-lat-lng").on("click",function(e){e.preventDefault(),$(this).attr("disabled","disabled"),$(".lat-lng-change-message").slideUp(),$("#gmb_lat_lng-latitude").val($(this).attr("data-lat")),$("#gmb_lat_lng-longitude").val($(this).attr("data-lng"))}),
//Add New Marker
